...
const memoriesCollectionRef = collection(db, 'week12_tugas_memories');
...
const insertNewMemory = async (title: string, type: 'good' | 'bad', location: { lat: number, lng: number }, fileName: string, photoBlob: Blob) => {
  const storageRef = ref(storage, `week12_tugas_uploads/${fileName}`);
...
const firebaseService = { getAllMemories, insertNewMemory };
